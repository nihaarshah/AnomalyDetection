...
class VAE(nn.Module):
    ...
    def __init__(self, input_size, encoder_sizes, decoder_sizes, z_size, batch_norm, dropout):
        super().__init__()
    ...
    def _set_encoder(self):
        ...
        for l in range(1, len(self.encoder_sizes)):
            layers.append(
                nn.Linear(self.encoder_sizes[l-1], self.encoder_sizes[l]))
        ...
        self.z_var = nn.Sequential(
            nn.Linear(self.encoder_sizes[-1], self.z_size),
            nn.Softplus()
        )
    ...
    def _set_decoder(self):
        ...
        for l in range(1, len(self.decoder_sizes)):
            layers.append(
                nn.Linear(self.decoder_sizes[l-1], self.decoder_sizes[l]))
            layers.append(nn.ReLU())
